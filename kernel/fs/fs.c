...
int fs_mount(const char* device_name, const char* path, const void* data)
{
    if ( strcmp(device_name, fat_fs.ops->dev_name) == 0 ) {
        strncpy((char*)fat_fs.path, path, sizeof(fat_fs.path));
        return fat_fs.ops->mount(&fat_fs, path);
    } else {
        printk("fs_mount: Unknown device name\n");
        return -STATUS_EIO;
    }
} 
...
{
    return fat_fs.ops->getdents(fd, dirp, count);
}
